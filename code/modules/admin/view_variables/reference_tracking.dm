...
/proc/type_but_also_for_lists(thing)
	if(isdatum(thing))
		var/datum/datum_thing = thing
		return datum_thing.type
	else if(islist(thing))
		return "/list\[[length(thing)]\]"
	else
		return "other ([thing])"

/datum/proc/_search_references()
	__search_references(src, &src.references_to_clear)

/proc/__search_references(source, references_to_clear)
	var/_type = type_but_also_for_lists(source)
	log_reftracker("Beginning search for references to a [_type], looking for [*references_to_clear] refs.")

	var/starting_time = world.time
	//Time to search the whole game for our ref
	_DoSearchVar(source, GLOB, "GLOB", starting_time, references_to_clear = references_to_clear) //globals
	...
	_DoSearchVar(source, global_vars, "Native Global", starting_time, references_to_clear = references_to_clear)
	log_reftracker("Finished searching native globals")
	if(*references_to_clear == 0)
		return

	for(var/datum/thing in world) //atoms (don't beleive its lies)
		_DoSearchVar(source, thing, "World -> [thing.type]", starting_time, references_to_clear = references_to_clear)
		if(*references_to_clear == 0)
			break
	...
	for(var/datum/thing) //datums
		_DoSearchVar(source, thing, "Datums -> [thing.type]", starting_time, references_to_clear = references_to_clear)
		if(*references_to_clear == 0)
			break
	...
	for(var/client/thing) //clients
		_DoSearchVar(source, thing, "Clients -> [thing.type]", starting_time, references_to_clear = references_to_clear)
		if(*references_to_clear == 0)
			break
	...
	log_reftracker("Completed search for references to a [_type].")

/datum/proc/DoSearchVar(potential_container, container_name, search_time, recursion_count, is_special_list)
	_DoSearchVar(src, potential_container, container_name, search_time, recursion_count, is_special_list, &src.references_to_clear)

/proc/_DoSearchVar(source, potential_container, container_name, search_time, recursion_count, is_special_list, references_to_clear)
	...
	if(isdatum(potential_container))
		...
		if(isatom(datum_container))
			...
			if(isarea(datum_container))
				is_area = TRUE
		else if(IS_CLIENT_OR_MOCK(datum_container))
			is_client = TRUE
		for(var/varname in vars_list)
			var/variable = vars_list[varname]
			var/var_ref = ref(variable)
			if(islist(variable) && ((var_ref && source_ref) && var_ref != source_ref))
				//Fun fact, vis_locs don't count for references
				if(varname == "vars" || (is_atom && (varname == "vis_locs" || varname == "overlays" || varname == "underlays" || varname == "filters" || varname == "verbs" || (is_area && varname == "contents"))) || (is_client && (varname == "screen" || varname == "images" || varname == "verbs")))
					continue
				// We do this after the varname check to avoid area contents (reading it incures a world loop's worth of cost)
				if(!length(variable))
					continue
				_DoSearchVar(source, variable,\
					"[container_name] [datum_container.ref_search_details()] -> [varname] (list)",\
					search_time,\
					recursion_count + 1,\
					/*is_special_list = */ is_atom && (varname == "contents" || varname == "vis_contents" || varname == "locs"), references_to_clear = references_to_clear)
			else if(variable == source || (var_ref && source_ref && (var_ref == source_ref)))
				...
				if(SSgarbage.should_save_refs)
					...
					log_reftracker("Found [_type] [text_ref(source)] in [datum_container.type]'s [datum_container.ref_search_details()] [varname] var. [container_name]")
				#else
				log_reftracker("Found [_type] [text_ref(source)] in [datum_container.type]'s [datum_container.ref_search_details()] [varname] var. [container_name]")
				#endif
				*references_to_clear -= 1
				if(*references_to_clear == 0)
					log_reftracker("All references to [_type] [text_ref(source)] found, exiting.")
					return
				continue
		...
		for(var/element_in_list in potential_cache)
			...
			if(islist(element_in_list))
				if(length(element_in_list))
					_DoSearchVar(source, element_in_list, "[container_name] -> [element_in_list] (list)", search_time, recursion_count + 1, references_to_clear = references_to_clear)
			//Check normal entrys
			else if(element_in_list == source)
				#ifdef REFERENCE_TRACKING_DEBUG
				if(SSgarbage.should_save_refs)
					if(!found_refs)
						found_refs = list()
					...
					log_reftracker("Found [_type] [text_ref(source)] in list [container_name].")
				#else
				log_reftracker("Found [_type] [text_ref(source)] in list [container_name].")
				#endif
			...
			if(!isnum(element_in_list) && !is_special_list)
				...
				try
					...
					if(islist(assoc_val))
						if(length(assoc_val))
							_DoSearchVar(source, potential_container[element_in_list], "[container_name]\[[element_in_list]\] -> [assoc_val] (list)", search_time, recursion_count + 1, references_to_clear = references_to_clear)
					//Check assoc entry
					else if(assoc_val == source)
						#ifdef REFERENCE_TRACKING_DEBUG
						if(SSgarbage.should_save_refs)
							if(!found_refs)
								found_refs = list()
							...
							log_reftracker("Found [_type] [text_ref(source)] in list [container_name]\[[element_in_list]\]")
						#else
						log_reftracker("Found [_type] [text_ref(source)] in list [container_name]\[[element_in_list]\]")
						#endif
						*references_to_clear -= 1
