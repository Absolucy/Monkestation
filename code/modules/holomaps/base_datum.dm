...
/datum/station_holomap/proc/initialize_holomap(turf/turf, current_z_level, mob/user = null, reinit_base_map = FALSE, extra_overlays = list())
	bogus = FALSE
	location_turf = turf
...
/datum/station_holomap/proc/update_map(list/overlays_to_use = list())
	...
	if(location_turf?.z == map_z && SSmapping.level_has_all_traits(location_turf.z, list(ZTRAIT_STATION)))
